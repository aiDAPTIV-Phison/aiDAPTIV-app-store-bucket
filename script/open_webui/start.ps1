...
while (-not $isHealthy) {
    ...
    try {
        $response = Invoke-WebRequest -Uri $healthUrl -Method Get -TimeoutSec 2 -ErrorAction Stop -UseBasicParsing
        $response2 = Invoke-WebRequest -Uri $healthUrl2 -Method Get -TimeoutSec 2 -ErrorAction Stop -UseBasicParsing
        if ($response.StatusCode -eq 200 -and $response2.StatusCode -eq 200) {
            $isHealthy = $true
            Write-Host "Open WebUI is ready!"
            if ($OpenBrowser) {            
                $response3 = Invoke-WebRequest -Uri "$env:LLM_URL/chat/completions" -Method Post -ContentType 'application/json' -Body (@{model='';messages=@(@{role='user';content='test'})} | ConvertTo-Json -Compress) -ErrorAction Stop -UseBasicParsing
                $response4 = Invoke-WebRequest -Uri "$env:EMBEDDING_URL/embeddings" -Method Post -ContentType 'application/json' -Body (@{model='';input='test'} | ConvertTo-Json -Compress) -ErrorAction Stop -UseBasicParsing
